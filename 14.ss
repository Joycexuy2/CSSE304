...
;    ;(display bodies)
;    (cond
; ;     [(null? (cdddr bodies)) (lit-exp #f)]
;      [(null? bodies) (lit-exp #f)]
;      [(null? (cdr bodies)) (syntax-expand (car bodies))]
;      [else (if (null? (cdr bodies))
;                (if-else-exp (syntax-expand (car bodies)) (syntax-expand (car bodies)) (lit-exp #f))
;                (syntax-expand (let-exp (list (syntax-expand (car bodies))) 
;                  (list (if-else-exp (car bodies) (car bodies) (or-loop (cdr bodies)))))))])))
...
(define case-loop
  (lambda (tests vars bodies)
    (cond-exp
      [(null? vars) '()]
      [(eq? (car vars) 'else) (car bodies)]
      [(contains? (eval-exp (parse-exp tests) empty-env) (car vars)) (car bodies)]
      [(not (null? vars)) (case-loop tests (cdr vars) (cdr bodies))])))
