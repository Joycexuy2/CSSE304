...
(define-datatype environment environment?
  (empty-env-record)
  (extended-env-record
   (syms (lambda (x) (or (symbol? x) (list-of symbol?))))
...
(define unparse-exp
  (lambda (e)
    (cases expression e
      ...
      [lambda-exp-improper (var opt body) 
        (cons 'lambda (cons (map unparse-exp var) (cons opt (map unparse-exp body))))]
      [lambda-exp (var body) 
...
(define *prim-proc-names* '(+ - * / add1 sub1 cons = < > <= >= not zero?
    ...
    symbol? set-car! set-cdr! vector-set! display newline apply map append list-tail))
