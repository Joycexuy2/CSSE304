...
(define apply-env
    (lambda (env sym succeed fail) ; succeed and fail are procedures applied if the var is or isn't found, respectively.
        (cases environment env
            ...
            (recursively-entended-env-record (proc-name idss bodies old-env)
              (let
                ((pos (list-find-position sym proc-name)))
                (if (number? pos)
                  (closure
                    (list-ref idss pos)
            ...
            (recursively-entended-env-record-improper (proc-name idss bodies old-env)
                (let
                    ((pos (list-find-position sym proc-name)))
                    (if (number? pos)
